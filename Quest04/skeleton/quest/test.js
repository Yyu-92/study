...
let folder = '';
let folderArr = [];
let folderWindow = document.getElementsByClassName('folderWindow');
...
function doubleClick(){
	let dbcItem = document.getElementsByClassName('folder');
	for(let i = 0; i < dbcItem.length; i++){
		dbcItem[i].addEventListener('dblclick', function(){
			let desktop = document.getElementsByClassName('desktop')[0];
			let folderWindow = document.createElement('div');
			folderWindow.setAttribute('class', 'folderWindow');
			folderWindow.innerText = '폴더입니다^^휴;;';
			desktop.append(folderWindow);
			
			return folderWindow;
		})	
		movingIcon(folderWindow[0]);
	}

	
	//console.log('folder : ' + dbcItem[0]);
}
