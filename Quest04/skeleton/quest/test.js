...
function createIcon(name){
	name.addEventListener('click', function(){
		folderValue = (this.id == 'folderSubmit') ? Number(document.getElementById('folderConst').value) : Number(document.getElementById('iconConst').value);
		console.log(document.getElementById('folderConst').value);
		for(let i = 0; i < folderValue; i++){
			let icon = document.createElement('img');
			icon.src = (this.id == 'folderSubmit') ? 'images/folder.png' : 'images/file.png';
			icon.alt = (this.id == 'folderSubmit') ? 'folder' : 'file';
			icon.class = icon.alt;
...
//createIcon(submitBtn);
//createIcon(iconSubmit);

submitBtnArr.forEach(createIcon);
